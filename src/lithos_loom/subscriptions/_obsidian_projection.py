"""``obsidian-projection`` subscription handler.

Turns ``lithos.task.*`` events into a markdown file at
``vault_path / tasks_file`` that the Obsidian Tasks plugin can query.

**Event vocabulary:** the Lithos SSE source emits
``{created,claimed,released,completed,cancelled}`` — there is no
``updated`` event today. We subscribe to all six so the projection
sees claim transitions (which determine actionability), plus ``updated``
as a forward-compatibility no-op should the source ever emit it.
...
historical record, not work. The base line shape —
...
week" Tasks-plugin queries work without polluting the open-task
inbox. A TTL sweep runs at the top of every event handle — cheap
dict walk, no new timer infrastructure; long-quiet daemons get a
...
File writes are layered to minimise disk I/O:
...
class _StateEntry:
    ...
    ``priority`` is the validated priority enum or ``None``, ``due_date``
...
for actionability: a task that was hidden because it was claimable-by-autonomous
...
_FILE_HEADER = (
    "%% Auto-generated by lithos-loom. Hand edits will be "
...
def make_handler(
    ...
    *,
    ...
    ``debounce_seconds`` controls write coalescing:
    ...
    # this session — otherwise the archiver would skip archiving work
    # the operator actually saw. The live render path never produced a
    # ``surfaced`` entry for it (no ``created`` event replays for an
    ...
    async def handle(event: Event, ctx: Any) -> None:
        ...
        # rewrite on the next event arrival. The archived sweep here
        # lets an archive of some OTHER task (set by the archiver on a
        # prior tick) force a flush so its line drops even when this
        # event didn't change the keyed entry. The same-event
        # immediate-evict path is handled at flush time, where the
    ...
    async def _flush() -> None:
        ...
        **Ordering — write first, sync_state second.** The disk
        ...
        self-write suppression keeps seeing consistent state.
        ...
        # subscription (a sibling handler on the same loop) had set
        # ``archived[id]`` — so the task survived the handle-time sweep.
        # By the time this debounced flush runs (~50ms later) the
        ...
        # Priority map. Only open entries carry a priority; resolved
        # entries always map to ``None`` because the renderer drops the
        # priority emoji on the resolved-line shape — surfacing them
        # here would let the watcher diff against a stale
        # projection-known priority after a task resolves.
        priority_markers = {
            tid: (entry.priority if entry.status == "open" else None)
            for tid, entry in state.items()
        }
        # Due-date map. Same shape as the priority map: open entries
        # carry the rendered date string (or ``None``); resolved
        # entries always map to ``None`` because the renderer drops
        # the 📅 marker on resolved lines.
        due_date_markers = {
        ...
        # Mark every task just written to disk as surfaced. Set HERE —
        # on the successful write — not in ``handle``, so the flag
        # means exactly "this task's line was written into tasks.md".
        # A task that opened and resolved inside one debounce window
        # without its open line ever being written is therefore not
        # flagged, and the archiver correctly skips it. The hash-skip
        # early-return above is fine: identical content means these ids
...
def _evict_expired(
    ...
    handle time and again at flush time. Keeping the two evictions in
    separate single-responsibility functions avoids the "which branch
    evicted this" ambiguity of a combined predicate.
...
def _evict_archived(
    ...
    Runs at BOTH handle time and flush time:
...
def _render_file(state: dict[str, _StateEntry]) -> str:
    ...
    tests and for the content-hash dedup at the file boundary).
    """
    if not state:
        return _FILE_HEADER + "\n"
    lines = [_FILE_HEADER, ""]
...
def _hash_existing_file(path: Path) -> bytes | None:
    ...
    (seeded on restart so a quiet-KB restart skips the write).
    """
    try:
        raw = path.read_bytes()
    except (FileNotFoundError, OSError):
...
# write_file_atomic lives in ``._atomic_write`` so the project-context
# projection can share the same temp + fsync + rename contract — see
# :mod:`lithos_loom.subscriptions._atomic_write` for the load-bearing
# invariants this projection's _flush relies on.
