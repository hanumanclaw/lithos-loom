...
**PR-merge close (#87).** The same reconcile sweep also completes non-issue-linked tasks on PR merge. For each open task it sees carrying `metadata.develop_pr_url` **and not** `metadata.github_issue_url` (issue-linked tasks already close via the issue mirror above), it fetches the PR (`GET /repos/{owner}/{repo}/pulls/{n}`) and acts on the state: **merged** → `lithos_task_complete` (idempotent — `task_not_found` on an already-terminal task is swallowed); **closed without merging** or **deleted (404)** → leave the task open and post a one-shot `[DeliveredPRClosed]` finding (a human decides whether to abandon); **still open** → no-op (re-polled next sweep); a **malformed** `develop_pr_url` → one-shot `[Friction]`. A de-dup marker scoped to the PR it resolved — `metadata.develop_pr_merge_state` (`merged` / `closed_unmerged` / `gone` / `unparseable`) paired with `metadata.develop_pr_merge_url` — de-dups across sweeps: the task is skipped only when its state is terminal **and** the recorded url still equals the current `develop_pr_url`, so neither the completion nor the finding fires twice (a still-open PR leaves the marker unset). The url scoping matters for the recovery path: when a rejected PR is abandoned and the task is re-developed into a **replacement** PR, `develop_pr_url` changes, the recorded url no longer matches, and the sweep re-evaluates the new PR — a stale marker never strands the task on a superseded PR. Transient GitHub failures (auth / 5xx / rate-limit) leave the marker unset and retry next sweep. The mechanism keys off `develop_pr_url` only, so any PR-producing plugin that records it gets merge→close for free; gated by `[github_watcher].pr_merge_poll_enabled` (default on), and never auto-cancels (abandoning a PR stays a human decision).
...
- **Config lookup.** Reviewer config is resolved from the project-context doc's metadata at `projects/<slug>/<slug>-project-context.md` (slug from `task.metadata.project`; fallback: lexicographically-smallest `project-context`-tagged doc under `projects/<slug>/`). Keys: `develop_reviewers` (pool of `{name, tool, block_threshold?, system_prompt?, fallback_chain?, model?, effort?}`), `develop_default_reviewers` (names that run when the task doesn't override), `develop_coder` (`{tool?, model?, effort?}`), `develop_fallback_chain`, `develop_max_rounds`, `develop_max_cost_usd`, `develop_image` (the sandbox container image for **every** agent + the test gate, e.g. `"ghcr.io/acme/dev:2026-06"`; default `ralph-sandbox:latest`), `develop_test_command` (test-gate command, **trusted as-is** — no auto-detection or tool-probe), `develop_test_gate` (bool; `false` excludes the `test` check from the per-round check-set — #131/ADR 0003 §10; with today's single-`test` default set that is observably "no gate"), `develop_block_on_red` (bool; a red/timeout gate blocks approval — default records it non-blocking) (#127). Per-task override: `task.metadata.reviewers` (names from the pool); `task.metadata.develop_model` / `task.metadata.develop_effort` (override the **coder's** model / reasoning effort for that one task — reviewer models stay project policy); `task.metadata.develop_image` (override the project's sandbox image for that one task — e.g. a task needing a heavier toolchain); `task.metadata.develop_test_command` / `develop_test_gate` / `develop_block_on_red` (override the test gate for that one task). Every miss — no slug, no doc, no keys, unknown name — degrades to the built-in single `code-quality` reviewer with a `[Friction]` finding; a populated pool without a default selection still runs only the built-in reviewer (pool membership does not auto-run).
...
develop_block_on_red: true                # a red test gate blocks approval (optional)
develop_coder:
  model: sonnet
  effort: medium
develop_reviewers:
...
| block on red gate | `task.develop_block_on_red` → `develop_block_on_red` (project) → `--block-on-red` (route fallback) → off (record, don't block) |
